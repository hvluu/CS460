import java.math.BigInteger;
import java.util.Scanner;

public class AESKeyExpansion {

    // S-box table
    public static int[] sBox
            = {
                };

    // Rcon 
    private static final int[] rcon
            = {
                
            };

    public static void main(String args[]) {
        int keySize = 4 * 4;
        byte[] key = new byte[keySize];
        int expandedKeySize = 44 * 4;
        byte[] w = new byte[expandedKeySize];

        String key1 = "";
        Scanner input = new Scanner(System.in);

        System.out.println("AES Key Expansion demo");
        System.out.println("======================");
        System.out.print("Please input a 32-bit key: ");
        key1 = input.nextLine();
        System.out.println();

        // Convert key to byte array
        key = new BigInteger(key1, 16).toByteArray();

        // Initialize first words with key
        for (int i = 0; i < key.length; ++i) {
            w[i] = key[i];
        }

        byte[] temp = {0, 0, 0, 0};

        int i = keySize;
        while (i < expandedKeySize) {

            //get last word of set
            for (int j = 0; j < 4; j++) {
                temp[j] = w[i - 4 + j];
            }

            //if completed a set of words
            if (i % keySize == 0) {
                rotateWord(temp);
                subWord(temp);
                //temp xor Rcon[i/4]
                temp[0] = (byte) (temp[0] ^ rcon[i / 16]);
            }

            //w[i]=w[i-4] xor temp
            for (int j = 0; j < 4; j++) {
                w[i + j] = (byte) (w[i - keySize + j] ^ temp[j]);
            }

            i += 4;
        }

        //display result in hex
        StringBuilder sb = new StringBuilder();
        for (byte b : w) {
            sb.append(String.format("%02X ", b));
        }
        System.out.println("New key after expanded:");
        System.out.println(sb.toString());

    }

    //Replace byte with value from S-Box
    private static void subWord(byte[] word) {
        for (int i = 0; i < 4; ++i) {
            word[i] = (byte) sBox[word[i] & 0xff];
        }

    }

    //Rotate bytes
    private static void rotateWord(byte[] word) {
        byte[] temp = new byte[4];

        for (int i = 0; i < word.length - 1; i++) {
            temp[i] = word[i + 1];
        }

        temp[word.length - 1] = word[0];

        System.arraycopy(temp, 0, word, 0, word.length);

    }
}